...
Define what counts as an integration test in this project, which
integration-test families exist, what each family proves, and how integration
testing differs across levels:
...
This skill does **not** define xUnit structure, fixture mechanics, or
executable test code shape. It defines the conceptual taxonomy only.

---

## Core definition

An integration test verifies the correctness of a **real assembled
collaboration** across one or more architectural boundaries.
...
- **inside boundary** — real assembled artifacts under test
- **outside boundary** — replaced, simulated, stubbed, or delegated
  collaborators
...
Integration at `unt` level verifies collaboration between multiple units
inside one component boundary.
...
Integration at `cmp` level verifies collaboration across the boundary of a
software component or domain subassembly.

#### Main purpose

Prove that the component-level architecture collaborates correctly through its
defined interfaces and realized local boundary stack.
...
Integration at `sys` level verifies collaboration between real system
components assembled according to the system architecture.
...
Correctness of integrated system-component collaboration against system
architecture.
...
Integration at `sos` level verifies collaboration between the system of
interest and external systems, devices, platforms, or operational environment
elements.

#### Main purpose

Prove cross-system compatibility and interaction correctness at the
...
Verify one concrete architectural interaction across a single connector or
boundary.
...
- real boundary behavior under burst, duplicate, overload, or concurrency
  conditions
...
Verify translation and realization behavior at a deployment or protocol
boundary.

#### Typical scope

- one adapter and the real boundary representation it translates to or from
- optionally guard/provider behind or in front of it, if part of the chosen
  slice
...
- wiring artifacts (`IServiceCollection` extensions)
- bootstrap artifacts (`Program.cs` / `Host` composition)
...
This family is requirement-oriented and shall not replace
connector/protocol integration testing.

---

## Family selection rule

A test shall be classified by its **main evidence target**, not by how large
it is.

One test may exercise several concerns, but it shall still have one primary
family.
...
- isolated unit logic tests with substituted collaborators only
...
- API calls that assert only one method was called without checking boundary
  correctness
...
4. Every integration test shall identify which artifacts are real and which are
   replaced.
5. Every integration test shall identify its oracle sources.
6. Every integration test shall assert boundary-visible outcomes, not only
   internal implementation details.
7. Every integration test shall be traceable to design or requirement
   artifacts.
...
Integration tests are derived from the architecture and implementation design
already present in the project. They are not invented independently.
