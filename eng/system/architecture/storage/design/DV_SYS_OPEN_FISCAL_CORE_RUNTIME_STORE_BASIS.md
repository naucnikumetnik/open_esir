...
| --- | --- | --- | --- |
| `E_APPROVED_CONFIGURATION_RECORD` | `derived_from_current_architecture` | `eng/system/architecture/dynamic/design/S_SYS_BOOTSTRAP_AND_CONFIGURATION.puml`; `eng/system/architecture/deployment/design/PC_SYS_OPEN_FISCAL_CORE_*.yaml`; `eng/system/architecture/structural/design/SV_SYS_OPEN_FISCAL_CORE.yaml` | Stored record is internal; it wraps approved system config selected and applied during bootstrap and command processing. In hosted access, authority mode is `cloud_primary_edge_cached`: cloud copy is authoritative and edge copy is cached projection unless explicit edge-cached fallback mode is active. |
| `E_ENVIRONMENT_BINDING_RECORD` | `derived_from_current_architecture` | `eng/system/architecture/dynamic/design/S_SYS_BOOTSTRAP_AND_CONFIGURATION.puml`; `eng/system/architecture/dynamic/design/S_SYS_STATUS_AND_OPERATOR_RECOVERY.puml`; `eng/system/architecture/deployment/design/PC_SYS_OPEN_FISCAL_CORE_*.yaml` | Needed to keep current environment, endpoint context, and last known good binding explicit instead of leaving it implicit inside config blobs. In hosted access, authority mode is `cloud_primary_edge_cached`: cloud binding is primary and edge binding is a projected cache unless explicit edge-cached fallback mode is active. |
| `E_SHARED_TAXCORE_SNAPSHOT_RECORD` | `derived_from_current_architecture` | `eng/system/architecture/dynamic/design/S_SYS_BOOTSTRAP_AND_CONFIGURATION.puml`; `eng/system/architecture/structural/design/SV_SYS_OPEN_FISCAL_CORE.yaml` | Stored shape is internal, but the snapshot content is derived from published shared TaxCore responses and bootstrap flow. In hosted access, the edge snapshot is cache and local-restore input, not canonical source of truth. |
...
| `E_LOCAL_JOURNAL_RECORD` search and operator enrichment fields | `reference_only` | `eng/docs/sources/Taxcore-MobilePOS-Android/app/src/main/java/online/taxcore/pos/data/realm/Journal.kt`; `eng/docs/sources/Taxcore-MobilePOS-Android/app/src/main/java/online/taxcore/pos/data/local/JournalManager.kt` | `buyerTin`, `transactionType`, `paymentType`, `invoiceType`, `buyerCostCenter`, and `invoiceItemsData` are treated as reference-backed enrichments, not normative fiscal protocol fields. They are optional, non-normative, and not required for fiscal-core correctness. |
| `E_AUDIT_PACKAGE_RECORD` | `core_official` | `eng/docs/sources/fiscal_catalog/working/finalized_types.csv`; `eng/docs/sources/fiscal_catalog/working/finalized_fields.csv`; `eng/docs/sources/fiscal_catalog/working/finalized_contracts.csv`; `eng/docs/sources/Technical-Documentation.pdf` | Canonical payload is the published `AuditPackage` DTO with `key`, `iv`, and `payload`. For local audit, audit packages remain in E-SDC local memory until a Proof-of-Audit command is received. |
...
| `E_AUDIT_REQUEST_PAYLOAD_RECORD` | `core_official` | `eng/docs/sources/fiscal_catalog/working/finalized_types.csv`; `eng/docs/sources/fiscal_catalog/working/finalized_contracts.csv`; `eng/docs/sources/fiscal_catalog/canonical_catalog.md` | Uses canonical `AuditRequestPayload`, explicitly distinct from `ProofOfAuditRequest`. This is an APDU-side opaque byte payload; base64 applies only inside the published HTTP `ProofOfAuditRequest` wrapper. |
...
| `E_PROOF_ARTIFACT_RECORD` | `core_official` | `eng/docs/sources/fiscal_catalog/working/finalized_types.csv`; `eng/docs/sources/fiscal_catalog/working/finalized_contracts.csv`; locally normalized `REF-005` evidence in `eng/docs/sources/fiscal_catalog/working/finalized_types.csv` and `eng/docs/sources/fiscal_catalog/working/manual_review_progress.csv` | Uses canonical `ProofOfAudit` primitive. This remains an opaque APDU proof payload at the storage-type level, not a JSON or HTTP-wrapper field. Local `REF-005` normalization is only to keep the supporting reference stable inside this slice. |

## Group C - Command, Recovery, And Export

| Entity or rule slice | Basis class | Primary sources | Notes |
| --- | --- | --- | --- |
| `E_BACKEND_AUTH_CONTEXT` | `derived_from_current_architecture` | `eng/docs/sources/fiscal_catalog/working/finalized_types.csv`; `eng/docs/sources/fiscal_catalog/working/finalized_fields.csv`; `eng/docs/sources/fiscal_catalog/working/finalized_contracts.csv`; `eng/system/architecture/dynamic/design/S_SYS_BACKEND_SYNC_AND_COMMAND_LIFECYCLE.puml` | Persisted auth-context storage is an internal cache projection, even though its payload uses the published `AuthenticationTokenResponse` and its lifetime follows `expiresAt`. |
...
| `E_STAGED_MEDIA_ARTIFACT_RECORD` | `derived_from_current_architecture` | `eng/system/architecture/dynamic/design/S_SYS_LOCAL_AUDIT_EXPORT.puml`; `eng/docs/sources/Technical-Documentation.pdf` | Logical staging wrapper is internal. When artifacts are materialized to removable media, official local-audit filename conventions apply for audit packages, commands, results, and ARP artifacts. |
...
| Delete local audit package on status `4` | `core_official` | `eng/docs/sources/fiscal_catalog/working/finalized_rules.csv`; `eng/docs/sources/fiscal_catalog/working/finalized_errors_statuses.csv`; `eng/docs/sources/fiscal_catalog/working/finalized_contracts.csv` | Applies to backend submission resolution in `AP_RESOLVE_AUDIT_BACKLOG_DISPOSITION`; local-audit export retention remains separately constrained by Proof-of-Audit handling. |
| Retry audit package only on status `1` | `core_official` | `eng/docs/sources/fiscal_catalog/working/finalized_rules.csv`; `eng/docs/sources/fiscal_catalog/working/finalized_errors_statuses.csv`; `eng/docs/sources/fiscal_catalog/working/finalized_contracts.csv` | Applies to backend submission resolution; other audit statuses are retained or held, not automatically retried. |
| During local audit, audit packages remain in E-SDC local memory until a Proof-of-Audit command is received | `core_official` | `eng/docs/sources/Technical-Documentation.pdf`; `eng/docs/sources/Concepts.pdf` | This constrains local-audit/export retention. Media transfer or export success alone does not authorize package deletion. |
...
| Audit package export filename `{UID}-{UID}-{Ordinal_Number}.json` | `core_official` | `eng/docs/sources/Technical-Documentation.pdf` | The local-audit file-based communication section explicitly publishes this audit-package naming convention. |
| Auxiliary removable-media filenames `{UID}.commands`, `{UID}.results`, `{UID}.arp` | `core_official` | `eng/docs/sources/Technical-Documentation.pdf` | The local-audit file-based communication section explicitly publishes these root and per-UID artifact names. |
...
- Hosted access uses cloud authority for hosted configuration unless explicitly operating in edge-cached fallback mode.
